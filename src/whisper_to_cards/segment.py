...
def _should_break(prev_text: str, curr_text: str) -> bool:
    cues = (r"^so[, ]", r"^in (conclusion|summary)", r"^next[, ]", r"^now[, ]",
            r"^okay[, ]", r"^let'?s")
...
def segment_transcript(transcript_path: Path, max_chars: int = 1200, max_segments: int = 200) -> List[Section]:
    ...
    def flush(end_idx: int):
        ...
        title = re.split(r"[.!?]", text, maxsplit=1)[0][:70].strip() or f"Section {len(sections)+1}"
        sec = Section(
...
def write_sections(sections: List[Section], out_path: Path, meta: dict | None = None) -> Path:
